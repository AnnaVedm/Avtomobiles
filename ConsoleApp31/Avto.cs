...
namespace МашинкиСупер
{
    class Avto
    {
        ...
        private double max_benzin_bak;              
        private double benzin_bak_volume;           //Текущий объем бака
        private double rashod;                      //Расход 100 км
        private double fullTime = 0;                
        private double distance;                    
        private double proehali = 0;       
        private double canDrive;                    //Можно проехать при текущем объёме бака
        private double coordinatesDistance = 0;     
        private double startProbeg;                 
        private double fullProbeg;                  
        ...
        private void Probeg(Avto[] cars, int i)
        {
            cars[i].fullProbeg += Math.Round(cars[i].canDrive, 3);
        }

        private void time(Avto[] cars, int i)  //Время в пути
        {
        ...
        private double zapravka(Avto[] cars, int i)
        {
        ...
        private void Coordinates(double CanDrive) //Перемещение по координатам
        {
            coordinatesDistance += CanDrive * 3;  //1 км пути = 3 пунктам по оси X
        }

        private void DriveTime(double speed, double CanDrive, ref double fullTime) //Время в пути
        {
